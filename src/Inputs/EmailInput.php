 ...
 */
class EmailInput extends InputTag implements InputInterface, DisableableInput, SelfValidatingInput
{
    ...
    {
        parent::__construct(TypeValue::email);
        $this->form_name = $name;
        $this->form_label = $label;
        $this->classes()->add('smol-email');
    }
    ...
    {
        if (!filter_var($this->formValue(), FILTER_VALIDATE_EMAIL))
