 ...
 */
class UrlInput extends InputTag implements InputInterface, DisableableInput, SelfValidatingInput
{
    ...
    {
        parent::__construct(TypeValue::url);
        $this->form_name = $name;
        $this->form_label = $label;
        $this->classes()->add('smol-url');
        $this->setPlaceholder('https://');
    }
    ...
    {
        if (!filter_var($this->formValue(), FILTER_VALIDATE_URL))
